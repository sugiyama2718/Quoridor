...
if __name__ == "__main__":
    ...
    max_win_rate_diff = 0.12
    joseki_num = 100
    n_cand = 3
    #n_div_try = 10

    #target_dir = KIFU_DIR
    target_dir = os.path.join("other_records", "240329_3700kifu")
        ...
        for action_str, mirror_action_str, normalized_action_str, depth in zip(action_list, mirror_action_list, normalized_action_list, range(len(action_list))):
            ...
            if depth <= max_depth:
                ...
                if key not in node.children.keys():
                    node.children[key] = get_opening_node_from_state(state, statevec2node)
    ...
    def traverse(tree, actions):
        if tree.visited_num / tree.game_num >= 0.01:
            print(actions)
    ...
    while True:
        ...
        for action in node.children.keys():
            #child = move_to_child(node, action, statevec2node)
